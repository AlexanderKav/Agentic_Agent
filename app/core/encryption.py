...
class DatabaseEncryption:
    ...
    def _init_cipher(self) -> None:
        ...
        # Production requires a secure key
        if not master_key and environment == 'production':
            raise ValueError(
                "DB_ENCRYPTION_KEY must be set in production! "
    ...
    def enable_pgcrypto_extension(self, db: Session) -> None:
    ...
    def encrypt_column(self, value: str) -> Optional[bytes]:
        """
    ...
    def decrypt_column(self, encrypted_value: bytes) -> Optional[str]:
        """
...
__all__ = ['DatabaseEncryption', 'get_db_encryption']